...
export const ChallengeCreator20: React.FC<Props> = ({ onBack, language }) => {
  ...
  const [mode, setMode] = useState<'AI' | 'AUTO'>('AI');
       ...
       {step === 'CONFIG' ? (
           <div className="space-y-6 max-w-2xl mx-auto">
               ...
               <div className="grid grid-cols-2 gap-4">
                   <button 
                      onClick={() => setMode('AI')}
                      className={`p-6 rounded-2xl border-2 flex flex-col items-center gap-2 transition-all ${mode === 'AI' ? 'border-violet-500 bg-violet-50 text-violet-700' : 'border-slate-100 bg-white text-slate-400 hover:bg-slate-50'}`}
                   >
                       <Sparkles size={32} />
                       <span className="font-bold">AI Generator</span>
                       <span className="text-[10px] opacity-70">Create New & Save to Bank</span>
                   </button>
                   <button 
                      onClick={() => setMode('AUTO')}
                      className={`p-6 rounded-2xl border-2 flex flex-col items-center gap-2 transition-all ${mode === 'AUTO' ? 'border-orange-500 bg-orange-50 text-orange-700' : 'border-slate-100 bg-white text-slate-400 hover:bg-slate-50'}`}
                   >
                       <Layers size={32} />
                       <span className="font-bold">Auto Mix (2.0)</span>
                       <span className="text-[10px] opacity-70">Mix from Question Bank</span>
                   </button>
               </div>
