  ...
  useEffect(() => {
     ...
     if (evt?.enabled && evt.endsAt) {
         const interval = setInterval(() => {
             const diff = new Date(evt.endsAt!).getTime() - Date.now();
             if (diff <= 0) {
          ...
          return (
              <div className="space-y-6 pb-24">
                  ...
                  <div className="space-y-4 mb-6">
                      ...
                      {/* DISCOUNT EVENT WIDGET */}
                      <DashboardSectionWrapper id="discount_widget" label="Discount Event">
                      {settings?.specialDiscountEvent?.enabled && discountTimer && (
                          <div onClick={() => onTabChange('STORE')} className="mx-1 mb-4 bg-gradient-to-r from-rose-600 to-pink-600 p-4 rounded-2xl shadow-lg flex items-center justify-between cursor-pointer border border-white/20 relative overflow-hidden group animate-in slide-in-from-top-4">
                              <div className="relative z-10">
                                  <div className="flex items-center gap-2 mb-1">
                                      <Ticket size={18} className="text-white animate-bounce" />
                                      <span className="text-xs font-black text-white tracking-widest uppercase">{settings.specialDiscountEvent.eventName}</span>
                                  </div>
                                  <p className="text-xs text-rose-100 font-medium">Get {settings.specialDiscountEvent.discountPercent}% OFF Now!</p>
                              </div>
                              <div className="relative z-10 text-right">
                                  <span className="text-2xl font-black text-white font-mono">{discountTimer}</span>
                                  <p className="text-[9px] text-rose-200 uppercase font-bold tracking-widest">Time Left</p>
                              </div>
                              {/* Particles */}
